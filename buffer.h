...
#define BUFFER_PUSH_FULL_ACTION(buf, c, ret_val) \
    do                                           \
    {                                            \
        (ret_val) = 0;                           \
        (buf).data[(buf).head] = (c);            \
...
#define BUFFER_STRUCT_DEF(ELEM_TYPE, IDX_TYPE, SIZE) \
    struct                                           \
    {                                                \
        ELEM_TYPE data[SIZE];                        \
        IDX_TYPE head;                               \
...
#define buffer_occupancy(buf) (((buf).head >= (buf).tail ? 0 : buffer_size(buf)) + (buf).head - (buf).tail)

#define buffer_free_space(buf) buffer_size(buf) - 1 - buffer_occupancy(buf)

#define is_buffer_empty(buf) ((buf).head == (buf).tail)

#define is_buffer_full(buf) ((buf).head + 1 >= buffer_size(buf) ? (buf).tail == 0 : (buf).head + 1 == (buf).tail)

#define buffer_generic_push(buf, c, ret_val, full_action, pos_push_action) \
    do                                                                     \
    {                                                                      \
...
#define buffer_generic_pop(buf, c, empty_action, pos_pop_action) \
    do                                                           \
    {                                                            \
